                            ...
                            if [ $proxy == true ]; then
                                if [ $proxyauth == true ]; then
                                    ...
                                    if [ $? -ne 0 ]; then
                                        ...
                                        rm "$sfdl_logs/$ladepfad"_lftp_index.log
                                        pub_server=1
                                        lftp -p $port -u "$username","$password" -e "set ftp:use-feat no; set ssl:verify-certificate no; set ftp:proxy $proxytyp://$proxyip:$proxyport; set net:timeout 30; set net:reconnect-interval-base 5; set net:max-retries 2; set ftp:ssl-allow no; open && ls \"$i\" && exit" $host 2> $sfdl_logs/$ladepfad'_lftp_error.log' 1> $sfdl_logs/$ladepfad'_lftp_index.log'
                                    else
                                        pub_server=0
                                    fi
                                fi
                                ...
                                if [ $? -ne 0 ]; then
                                    ...
                                    rm "$sfdl_logs/$ladepfad"_lftp_index.log
                                    pub_server=1
                                    lftp -p $port -u "$username","$password" -e "set ftp:use-feat no; set ssl:verify-certificate no; set net:timeout 30; set net:reconnect-interval-base 5; set net:max-retries 2; set ftp:ssl-allow no; open && ls \"$i\" && exit" $host 2> $sfdl_logs/$ladepfad'_lftp_error.log' 1> $sfdl_logs/$ladepfad'_lftp_index.log'
                                else
                                    pub_server=0
                                fi
                            fi
        ...
        if [ $sfdl_lftp_download == true ]; then
            ...
            if hash lftp 2>/dev/null; then
                ...
                if [ $pub_server == 1 ]; then
                    ...
                    for i in "${filearray[@]}"
                    do
                        size_in_bytes=$(echo "$i" | sed 's/^[^ ]* [^ ]* [^ ]* [^ ]* [^ ]* *\([0-9]\+\).*/\1/')
                        size_in_mb=$(echo "scale=2; $size_in_bytes / (1024 * 1024)" | bc)
                        printf "%-20s %s\n" "$size_in_mb MB" "$(echo "$i" | sed 's/.* \(.*\)|.*/\1/g')"
                    done
                    echo
                fi
...
# Original von raz0r    lftp -p $port -u "$username","$password" -e 'set ftp:use-feat no; set ssl:verify-certificate no; set ftp:ssl-allow no; mirror --continue --parallel="'$maxdl'" "'$DLPATH'" "'$sfdl_downloads/$name'"; exit' $host > "$sfdl_logs/$name"_download.log | "$sfdl_sys/prog.sh" "$sfdl_downloads/$name" "$bsize" "$pwd" "${filearray[@]}"
